...
class GetData(PathMetadata):
    ...
    def get_REACTOME(self):
        ...
        try:
            with open(self.path_in_inside + '/reactome_jbio_dict.json', 'r') as json_file:
    ...
    def get_REF_GEN(self):
        ...
        """
        This method gets the REF_GEN which is the combination of Homo sapiens and Mus musculus genomes for scientific use.
        ...
        try:
            ...
            with open(self.path_in_inside + '/gene_dictionary_jbio_annotated.json', 'r') as json_file:
    ...
    def get_REF_GEN_RNA_SEQ(self):
        ...
        try:
            ...
            with open(self.path_in_inside + '/human_tissue_expression_HPA.json', 'r') as json_file:
                human_tissue_expression_HPA = json.load(json_file)
                        
            
            with open(self.path_in_inside + '/human_tissue_expression_RNA_total_tissue.json', 'r') as json_file:
                human_tissue_expression_RNA_total_tissue = json.load(json_file)
                
            
            
            with open(self.path_in_inside + '/human_tissue_expression_fetal_development_circular.json', 'r') as json_file:
                human_tissue_expression_fetal_development_circular = json.load(json_file)
                
            
            
            with open(self.path_in_inside + '/human_tissue_expression_illumina_bodyMap2.json', 'r') as json_file:
    ...
    def get_HPA(self):
        ...
        try:
            ...
            with open(self.path_in_inside + '/HPA_jbio_dict.json', 'r') as json_file:
    ...
    def get_DISEASES(self):
        ...
        try:
            ...
            #load diseases
            with open(self.path_in_inside + '/disease_jbio_dict.json', 'r') as json_file:
    ...
    def get_ViMIC(self, path_in_use = _path_in_inside):
        ...
        try:
            ...
            #load viral diseases
            with open(self.path_in_inside + '/viral_jbio_dict.json', 'r') as json_file:
    ...
    def get_KEGG(self):
        ...
        try:
            ...
            #load kegg
            with open(self.path_in_inside + '/kegg_jbio_dict.json', 'r') as json_file:
    ...
    def get_GO(self):
        ...
        try:
            ...
            with open(self.path_in_inside + '/goterm_jbio_dict.json', 'r') as json_file:
    ...
    def get_IntAct(self):
        ...
        try:
            with open(self.path_in_inside + '/intact_jbio_dict.json', 'r') as json_file:
    ...
    def get_STRING(self):
        ...
        try:
            with open(self.path_in_inside + '/string_jbio_dict.json', 'r') as json_file:
...
class Finder(Tools):
    ...
    def spec_dic(gene_dictionary, species):
        if species == 'Homo sapiens':
            ...
            tf_h = ['Rattus norvegicus' in x for x in gene_dictionary['species']]
        elif species == 'both1':
            tf_h = ['Homo sapiens' in x or 'Mus musculus' in x for x in gene_dictionary['species']]
        elif species == 'both2':
            tf_h = ['Homo sapiens' in x or 'Rattus norvegicus' in x for x in gene_dictionary['species']]
        elif species == 'both3':
            tf_h = ['Rattus norvegicus' in x or 'Mus musculus' in x for x in gene_dictionary['species']]
        elif species == 'all':
            tf_h = [True for x in gene_dictionary['species']]  # Selects all species
        else:
            raise ValueError("Invalid species specified.")
            
        ids = [value for value, flag in zip(gene_dictionary['sid'], tf_h) if flag]
        
        return ids
    
    coto1 = spec_dic(gene_dictionary = input_metadata, species = 'Mus musculus')

    
    def find_fetures_id(genome_dict, fetures_list):
    ...
    def v1_is_in_v2(v1:list, v2:list):
        
        v3 = [x for x in v1 if x in v2]
        
        return v3
    
    coto2 = find_fetures_id(genome_dict = input_metadata, fetures_list = ['KIT', 'HTT', 'MAPT','PAX6', 'PAX3'])


    t =   v1_is_in_v2(v1 = list(coto2['found_ids']), v2 = coto1)
    
    
    def return_dictionary(genome_dict, sid_list:list = []):
        if isinstance(genome_dict, dict):
            genome_dict = pd.DataFrame(genome_dict)
            genome_dict = genome_dict[genome_dict['sid'].isin(sid_list)]
            genome_dict = genome_dict.reset_index(drop = True)
            
            return genome_dict.to_dict(orient = 'list')


    gene_dictionary = return_dictionary(genome_dict = input_metadata, sid_list = t)
...
class Selection:
    ...
    def select_specificiti(self):
        
        with open(path_in_use + '/.json', 'r') as json_file:
            select = (json.load(json_file))
        ...
        for k in select.keys():
            ...
            tmp = pd.DataFrame(select[k])
            
            try:
                tmp['name'] = [x.capitalize() for x in  tmp['name']]
            except:
                pass
            
            tmp = tmp[tmp['id'].isin(gene_dictionary['id_HPA'])]
            tmp = pd.merge(tmp, gene_dictionary[['id_HPA', 'sid', 'possible_names']], how = 'left', left_on = 'id', right_on = 'id_HPA')
            tmp = tmp.drop(['id_HPA'], axis = 1)
                                    
            tmp = tmp.rename(columns={'sid':'gene_id'})
            
            # additional for ABM
            # tmp = pd.merge(tmp,  epd_genes[['is_nc','sc_id']], how = 'left', on = 'sc_id')
            
            # epd = pd.read_csv('EPD/epd_promoters_export_202401272354.csv')
            # len(epd['tss'])
            # len(set(epd['tss']))

            
            # epd = epd.drop_duplicates()
            
            # tmp.to_csv(k + '.csv', index=False)

            full_dict[k] = tmp.to_dict(orient = 'list')
    ...
    def select_KEGG():
        ...
        with open(path_in_use + '/gopa_data.json', 'r') as json_file:
            select = (json.load(json_file))
            
        select = pd.DataFrame(select['KEGG'])

        gene_dictionary = pd.DataFrame(gene_dictionary)
        
        select = select[select['id'].isin(gene_dictionary['id_KEGG'])]

        
        self.KEGG = select.to_dict(orient = 'list')
    ...
    def select_GOTERM():
        ...
        select = select['GO-TERM']
        
        tmp = pd.DataFrame(select['gene_info'])
        
        gene_dictionary = pd.DataFrame(gene_dictionary)
        
        tmp = tmp[tmp['id'].isin(gene_dictionary['id_GO'])]
        
        tmp2 = pd.DataFrame(select['go_names'])
        tmp2 = tmp2[tmp2['GO_id'].isin(tmp['GO_id'])]
        ...
        select['go_names'] = tmp2.to_dict(orient = 'list')
        
        del tmp2        

        tmp2 = pd.DataFrame(select['hierarchy'])
        tmp2 = tmp2[tmp2['GO_id'].isin(tmp['GO_id'])]
        
        select['hierarchy'] = tmp2.to_dict(orient = 'list')
        select['gene_info'] = tmp.to_dict(orient = 'list')
    ...
    def select_REACTOME():
        ...
        select = pd.DataFrame(select['REACTOME'])
